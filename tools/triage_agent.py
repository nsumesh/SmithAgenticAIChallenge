"""
Triage Agent — multi-shipment urgency ranking.
...
Designed to run BEFORE the orchestrator on a batch of incoming
at-risk shipments, so the highest-risk ones are processed first.
"""
...
def _enrich_shipment(s: dict) -> dict:
    ...
    Finds the highest-risk window for this shipment and attaches:
      - hours_at_risk: windows in breach * 0.5hr each (windows are 30min)
      - peak_temp_c: hottest avg_temp recorded
      - breach_rule: most common det_rules_fired for this shipment
      - product_name: human-readable name from product_profiles.json
    """
    df = _get_scored()
...
def _execute(
    ...
    # Enrich with real data if requested and CSV is available
    ...
    for rank, s in enumerate(ranked, 1):
        ...
        priority_list.append({
            ...
            "hours_at_risk": s.get("hours_at_risk", None),
            "peak_temp_c": s.get("peak_temp_c", None),
            "primary_breach_rule": s.get("primary_breach_rule", ""),
            "windows_in_breach": s.get("windows_in_breach", None),
            "total_windows": s.get("total_windows", None),
        })
